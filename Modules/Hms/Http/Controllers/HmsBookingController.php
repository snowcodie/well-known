...
class HmsBookingController extends Controller
{
    ...
    public function calendar(Request $request){
        ...
        $rooms = HmsRoom::leftjoin('hms_room_types as type', 'type.id', '=', 'hms_rooms.hms_room_type_id')
                            ...
                            ->select('hms_rooms.*', 'type.type','type.is_vip', 'type.id as type_id')->orderBy('hms_rooms.id','DESC');
                            if($request->type_id){
    ...
    //dd($request);
        ...
        for ($i =0; $i<=6; $i++) {
                ...
                if($request->day_next){
                    ...
                    $date_html .= '<th style="width: 100px;" class="text-center '.$class.'">
                    '.$header_date->startOfWeek(Carbon::SUNDAY)->addDays($i)->format('d').' <br>
                    '.$header_date->startOfWeek(Carbon::SUNDAY)->addDays($i)->format("l") .'
                    </th>';
                }
                $class = '';
        }
        ...
        foreach($rooms as $room){
            if($room->is_vip==1){
                $color="background-color:#7EE8FA";
            }
            else{
                $color='';
            }
            $html .= '<tr><th style="'.$color.'" class='.$text.'>'.$room->room_number.' <br> <small>'.$room->type.'</small/></th>';
...
/*For showing reservation form for guest*/
  ...
  /*      if (auth()->user()->can('supplier.create')) {
        ...
        return view('hms::bookings.reservation', compact('status', 'extras', 'walk_in_customer', 'types', 'customer_groups', 'payment_line', 'payment_types', 'pos_settings', 'change_return', 'accounts'));
    }
